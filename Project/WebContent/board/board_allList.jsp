...
<title> SneakerHouse 전체글보기</title>
...
<body>
	...
	<%
		...
		int count = bdao.getAllBoardCount();
   		...
   		<div id="main_content">
    		<div id="bb_contentWrap">
    			<div id="bb_content">
							...
							<table class="main_table_collection">
					    		...
					    		<tbody>
						    		...
						    		<tr class="content_table_collection">
						    			<td> <%= bdto.getNum() %>  </td>
						    			<td> [<%= bdto.getBoard_type() %>] </td>
						    			<td class="main_content"><a href="board_allContent.jsp?board_type=<%=bdto.getBoard_type()%>&re_ref=<%= bdto.getRe_ref()%>&pageNum=<%=pageNum%>&name=<%=bdto.getMember_id()%>"> <%= bdto.getSubject() %> </a> </td>
    				...
    				<!-- 페이지 번호 파트 -->
			    	<div class="page_control" style="padding: 30px 0 30px;">
			    		<%
			    			//페이징 처리
			    			//글이 있을때만 처리
			    			if(count != 0) {
								...
								%>
								<p> <a href="board_allList.jsp?pageNum=<%= startPage - pageBlock %>"> <i class="fas fa-angle-left"></i>  </a> </p>
								<%
								}
								...
								<ol>
									<li>
										<a href="board_allList.jsp?pageNum=<%=i%>"><%= i %> </a>
									</li>
								</ol>
								<%
								}
								...
								if(endPage < pageCount) {
								%>
								<p> <a href="board_allList.jsp?pageNum=<%= startPage + pageBlock %>"> <i class="fas fa-angle-right"></i> </a> </p>
								<%
								}
			    			}
			    		%>
